...
Eigen::MatrixXd Controller::setDesiredVelocity(double linear_x, double linear_y, double angular_x, double angular_y,
                                    double angular_z, double position_z) {
    imu_angular_accelaration_ = (imu_angular_velocity_ - imu_angular_velocity_)/PERIOD;

    desired_velocity_ << linear_x, linear_y, position_z, angular_x, angular_y, angular_z;

    // Actual velocity to be got from the IMU and depth to be got from the pressure/depth sensor
    linear_velocity_ = linear_velocity_ + (imu_linear_accelaration_ + previous_imu_linear_accelaration_) * PERIOD * 0.5; //account for cummilation error and add reset

    current_velocity_ <<
                    imu_angular_velocity_.coeff(0,0),
            imu_angular_velocity_.coeff(1,0),
            imu_angular_velocity_.coeff(2,0),//position, not velocity
            linear_velocity_.coeff(0,0),
            linear_velocity_.coeff(1,0),
            linear_velocity_.coeff(2,0);

    current_acceleration_ <<
                        imu_angular_accelaration_.coeff(0,0),
            imu_angular_accelaration_.coeff(1,0),
            imu_angular_accelaration_.coeff(2,0), //to be changed from vel to osition
            imu_linear_accelaration_.coeff(0,0),
            imu_linear_accelaration_.coeff(1,0),
    ...
    y_ <<
       current_velocity_(0,0),
            ...
            current_acceleration_(2,0),
            current_acceleration_(3,0),
            current_acceleration_(4,0),
            current_acceleration_(5,0);

    intergrator_acumulator_ = intergrator_acumulator_ + ((desired_velocity_ + previous_desired_velocity_)/2  - (y_ + previous_y_)/2)*PERIOD;
    ...
    if (torque_matrix_(0,0) < 0) {
        pwm_matrix_(0, 0) = 1454 + 298 * torque_matrix_(0, 0) + 56.7 * torque_matrix_(0, 0) * torque_matrix_(0, 0);
    } else if (torque_matrix_(1,0) > 0) {
        pwm_matrix_(0,0) = 1543 + 179* torque_matrix_(0,0) - 16.5 * torque_matrix_(0,0) *torque_matrix_(0,0);
    } else {
        pwm_matrix_(1, 0) = 0;
    }

    if (torque_matrix_(1,0) < 0) {
        pwm_matrix_(1, 0) = 1454 + 298 * torque_matrix_(1, 0) + 56.7 * torque_matrix_(1, 0) * torque_matrix_(1, 0);
    } else if (torque_matrix_(1,0) > 0) {
        pwm_matrix_(1, 0) = 1543 + 179 * torque_matrix_(1, 0) - 16.5 * torque_matrix_(1, 0) * torque_matrix_(1, 0);
    } else {
        pwm_matrix_(1, 0) = 0;
    }

    if (torque_matrix_(2,0) < 0) {
        pwm_matrix_(2, 0) = 1454 + 104 * torque_matrix_(2, 0) + 5.1 * torque_matrix_(2, 0) * torque_matrix_(2, 0);
    } else if (torque_matrix_(2,0) > 0) {
        pwm_matrix_(2, 0) = 539 + 89.9 * torque_matrix_(2, 0) - 3.91 * torque_matrix_(2, 0) * torque_matrix_(2, 0);
    } else {
        pwm_matrix_(2, 0) = 0;
    }

    if (torque_matrix_(3,0) < 0) {
        pwm_matrix_(3,0) = 1454 + 104* torque_matrix_(3,0) + 5.1 *  torque_matrix_(3,0) * torque_matrix_(3,0);
    } else if (torque_matrix_(1,0) > 0) {
        pwm_matrix_(3, 0) = 539 + 89.9 * torque_matrix_(3, 0) - 3.91 * torque_matrix_(3, 0) * torque_matrix_(3, 0);
    } else {
        pwm_matrix_(2, 0) = 0;
    }
