...
ControllerNode::ControllerNode(int argc, char** argv, std::string name) {
    ...
    twist_subscriber_ = nh.subscribe("velocity_publisher", 1000, &ControllerNode::desiredVelocityCallback, this);
    imu_subscriber_ =  nh.subscribe("imu_data", 1000, &ControllerNode::imuCallback, this);
    arduino_publisher_ = nh.advertise<std_msgs::Int32MultiArray>("Arduino", 1000);
}
...
void ControllerNode::imuCallback(const sensor_msgs::Imu::ConstPtr &msg) {
    setImuDataHelper(controller_, msg->angular_velocity, msg->linear_acceleration);
}

Eigen::MatrixXd setDesiredVelocityHelper(Controller &controller, geometry_msgs::Vector3 linear, geometry_msgs::Vector3 angular, double position_z) {
...
void ControllerNode::desiredVelocityCallback(const nav_msgs::Odometry::ConstPtr &desired_twist_velocity) {
    Eigen::MatrixXd torque = setDesiredVelocityHelper(controller_, desired_twist_velocity->twist.twist.linear, desired_twist_velocity->twist.twist.angular, desired_twist_velocity->pose.pose.position.z);
