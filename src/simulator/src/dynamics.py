...
class Dynamics:
    ...
    def getConfig(self):
        ...
        self.num_actuators = rospy.get_param(self.vehicle_name
                + '/num_actuators')

        self.period = rospy.get_param(self.vehicle_name + '/dynamics'
                + '/period')
        self.mass = rospy.get_param(self.vehicle_name + '/dynamics'
                                    + '/mass')
        self.gravity_center = array(rospy.get_param(self.vehicle_name
                                    + '/dynamics' + '/gravity_center'))
        self.g = rospy.get_param(self.vehicle_name + '/dynamics' + '/g')
        self.radius = rospy.get_param(self.vehicle_name + '/dynamics'
                + '/radius')
        self.ctf = rospy.get_param(self.vehicle_name + '/dynamics'
                                   + '/ctf')
        self.ctb = rospy.get_param(self.vehicle_name + '/dynamics'
                                   + '/ctb')
        self.actuators_tau = rospy.get_param(self.vehicle_name
                + '/dynamics' + '/actuators_tau')
        self.actuators_maxsat = rospy.get_param(self.vehicle_name
                + '/dynamics' + '/actuators_maxsat')
        self.actuators_minsat = rospy.get_param(self.vehicle_name
                + '/dynamics' + '/actuators_minsat')
        self.actuators_gain = rospy.get_param(self.vehicle_name
                + '/dynamics' + '/actuators_gain')
        self.dzv = rospy.get_param(self.vehicle_name + '/dynamics'
                                   + '/dzv')
        self.dv = rospy.get_param(self.vehicle_name + '/dynamics'
                                  + '/dv')
        self.dh = rospy.get_param(self.vehicle_name + '/dynamics'
                                  + '/dh')
        self.density = rospy.get_param(self.vehicle_name + '/dynamics'
                + '/density')
        self.tensor = array(rospy.get_param(self.vehicle_name
                            + '/dynamics' + '/tensor'))
        self.damping = array(rospy.get_param(self.vehicle_name
                             + '/dynamics' + '/damping'))
        self.quadratic_damping = \
            array(rospy.get_param(self.vehicle_name + '/dynamics'
                  + '/quadratic_damping'))

        self.am = rospy.get_param(self.vehicle_name + '/dynamics'
                                  + '/allocation_matrix')

        self.p_0 = array(rospy.get_param(self.vehicle_name + '/dynamics'
                          + '/initial_pose'))
        self.v_0 = array(rospy.get_param(self.vehicle_name + '/dynamics'
                          + '/initial_velocity'))
        self.frame_id = rospy.get_param(self.vehicle_name + '/dynamics'
                + '/frame_id')
        self.external_force_topic = rospy.get_param(self.vehicle_name
                + '/dynamics' + '/external_force_topic')

#       Currents data

        self.current_mean = \
            array(rospy.get_param('dynamics/current_mean'))
        self.current_sigma = \
            array(rospy.get_param('dynamics/current_sigma'))
        self.current_min = array(rospy.get_param('dynamics/current_min'
                                 ))
        self.current_max = array(rospy.get_param('dynamics/current_max'
                                 ))

        self.uwsim_period = rospy.get_param(self.vehicle_name
                + '/dynamics/uwsim_period')
...
#        rospy.loginfo("s(): \n %s", x)
    ...
    def coriolisMatrix(self):
        s1 = self.s(dot(self.M[0:3, 0:3], self.v[0:3]) + dot(self.M[0:
                    3, 3:6], self.v[3:6]))
        s2 = self.s(dot(self.M[3:6, 0:3], self.v[0:3]) + dot(self.M[3:
                    6, 3:6], self.v[3:6]))
        c = zeros((6, 6))
        c[0:3, 3:6] = -s1
        c[3:6, 0:3] = -s1
...
#
    ...
    def thrustersDynamics(self, u):
        ...
        for i in range(size(u)):
            y[i] = (self.period * u[i] + self.actuators_tau[i]
                    * self.y_1[i]) / (self.period
                    + self.actuators_tau[i])
    ...
    def pubPose(self, event):
        ...
        orientation = \
            tf.transformations.quaternion_from_euler(self.p[3],
                self.p[4], self.p[5], 'sxyz')
        ...
        br.sendTransform((self.p[0], self.p[1], self.p[2]),
                         orientation, rospy.Time.now(), 'world',
                         str(self.frame_id))

    def computeTf(self, tf):
        r = PyKDL.Rotation.RPY(math.radians(tf[3]),
                               math.radians(tf[4]), math.radians(tf[5]))
    ...
    def __init__(self):
        ...
        if len(sys.argv) != 6:
            sys.exit('Usage: ' + sys.argv[0]
                     + ' <namespace> <input_topic> <output_topic>')
...
#        rospy.loginfo("Inverse Mass Matrix: \n%s", str(self.IM))
    ...
    # TODO: set the topic names as parameters
        ...
        rospy.Subscriber(self.input_topic, Float64MultiArray,
                         self.updateThrusters)
        rospy.Subscriber(self.external_force_topic, WrenchStamped,
