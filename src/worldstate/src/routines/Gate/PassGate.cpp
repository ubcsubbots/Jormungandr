...
PassGate::getNodeSubscriptions(ros::NodeHandle nh) {
    ...
    subs.push_back(nh.subscribe(gateDetectTopic, 10, &PassGate::gateDetectCallBack, this));

    subs.push_back(nh.subscribe(line_detect_topic, 10, &PassGate::lineDetectCallback, this));
    ...
    timer_ = nh.createTimer(ros::Duration(30),&PassGate::timerCallback, this ,false);
    timer_.start();
    return subs;
}
...
void PassGate::lineDetectCallback(const line_detect::LineDetectMsg::ConstPtr& msg){
    worldstate::StateMsg stateMsg;

    if(!(msg->lateralDistanceFromFrontMarker == -1.0f || msg->distanceFromEndOfFrontMarker == -1.0f || msg-> angleToParallelFrontMarker == -1.0f)){
