...
decisionNode::decisionNode(int argc, char** argv, std::string nodeName) {
    ...
    subscriber_ =
    nh.subscribe("/vision/output", 1, &decisionNode::subscriberCallBack, this);

    publisher_ = nh.advertise<geometry_msgs::Twist>(publishTopic, 1000);
}

void decisionNode::subscriberCallBack(
const gate_detect::gateDetectMsg gateDetectMsg) {
...
int main(int argc, char** argv) {
    ...
    decisionNode decisionNode1(argc, argv, node_name);
